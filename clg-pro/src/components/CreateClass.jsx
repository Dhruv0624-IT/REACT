import {
  Button,
  Dialog,
  DialogActions,
  DialogContent,
  DialogContentText,
  DialogTitle,
  TextField,
} from "@mui/material";

import React, { useState } from "react";
import { useAuthState } from "react-firebase-hooks/auth";
import { useRecoilState } from "recoil";
import { auth, db } from "../firebase.js";
import { createDialogAtom } from "../utils/atoms";

function CreateClass() {
  const [user, loading, error] = useAuthState(auth);
  const [open, setOpen] = useRecoilState(createDialogAtom);
  const [className, setClassName] = useState("");

  const handleClose = () => {
    setOpen(false);
  };

  const createClass = async () => {
    try {
      if (!className) {
        return alert("Please enter a class name.");
      }

      // Add a new class document to the 'classes' collection
      const newClass = await db.collection("classes").add({
        creatorUid: user.uid,
        name: className,
        creatorName: user.displayName,
        creatorPhoto: user.photoURL,
        posts: [],
      });

      // Find the user's document in the 'users' collection
      const userRef = await db
        .collection("users")
        .where("uid", "==", user.uid)
        .get();

      // Check if the user document exists before proceeding
      if (userRef.empty) {
        // Handle case where user document doesn't exist (e.g., create one)
        // For this example, we'll just alert and stop
        console.error("User document not found.");
        return alert("User data not found. Please log in again.");
      }

      // Get the document data and reference from the query result
      const userDoc = userRef.docs[0];
      const userData = userDoc.data();
      
      // Use the spread operator to correctly update the array
      const userClasses = [...userData.enrolledClassrooms, {
        id: newClass.id,
        name: className,
        creatorName: user.displayName,
        creatorPhoto: user.photoURL,
      }];

      // Update the user's enrolledClassrooms array
      await userDoc.ref.update({
        enrolledClassrooms: userClasses,
      });

      handleClose();
      alert("Classroom created successfully!");

    } catch (err) {
      console.error(err);
      alert(`Cannot create class - ${err.message}`);
    }
  };

  return (
    <div>
      <Dialog
        open={open}
        onClose={handleClose}
        aria-labelledby="form-dialog-title"
      >
        <DialogTitle id="form-dialog-title">Create class</DialogTitle>
        <DialogContent>
          <DialogContentText>
            Enter the name of class and we will create a classroom for you!
          </DialogContentText>
          <TextField
            autoFocus
            margin="dense"
            label="Class Name"
            type="text"
            fullWidth
            value={className}
            onChange={(e) => setClassName(e.target.value)}
          />
        </DialogContent>
        <DialogActions>
          <Button onClick={handleClose} color="primary">
            Cancel
          </Button>
          <Button onClick={createClass} color="primary">
            Create
          </Button>
        </DialogActions>
      </Dialog>
    </div>
  );
}

export default CreateClass;